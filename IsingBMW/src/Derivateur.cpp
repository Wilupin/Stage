...
//Given a,b,c[0..n-1], as output from routine chebft §5.8, and given n, the desired degree of approximation (length of c to be used), this routine returns the array cder[0..n-1], the Chebyshev coefficients of the derivative of the function whose coefficients are c.
{
  ...
  cder[Cheb->get_n()-1]=0.0; //n-1 and n-2 are special cases.
  cder[Cheb->get_n()-2]=2*(Cheb->get_n()-1)*c[Cheb->get_n()-1];
  for (j=Cheb->get_n()-3;j>=0;j--)
    cder[j]=cder[j+2]+2*(j+1)*c[j+1];// Equation (5.9.2).
  con=2.0/(Cheb->get_xmax()-Cheb->get_xmin());
  for (j=1;j<Cheb->get_n();j++) //Normalize to the interval b-a.
    {
      cder[j] *= con;
    }
  cder[0] *= 1.0/(Cheb->get_xmax()-Cheb->get_xmin());
}
...
//this routine returns the array cder[0..n-1], the Chebyshev coefficients of the derivative of the function whose coefficients are c.
{
  ...
  cder1[0] *= 1.0/(Cheb->get_xmax()-Cheb->get_xmin());
  cder2[0] *= 1.0/(Cheb->get_xmax()-Cheb->get_xmin());
}
...
//Given a,b,c[0..n-1], as output from routine chebft §5.8, and given n, the desired degree of approximation (length of c to be used), this routine returns the array cder[0..n-1], the Chebyshev coefficients of the derivative of the function whose coefficients are c.
{
  ...
  cder[Cheb->get_n()-1][iii]=0.0; //n-1 and n-2 are special cases.
  cder[Cheb->get_n()-2][iii]=2*(Cheb->get_n()-1)*c[Cheb->get_n()-1][iii];
  for (j=Cheb->get_n()-3;j>=0;j--)
  ...
  cder[0][iii] *= 1.0/(Cheb->get_xmax()-Cheb->get_xmin());
}
