...
void Chebychev::CoeffCheb2D(std::vector<std::vector<std::vector<double>>> & fff,
			    std::vector<std::vector<std::vector<double>>> & coef_fff,
			    int ir)
{
	...
	for(int j = 0; j <= i; j++)
	  {
	    ...
	    // Somme double sur les racines du polynomes Tn
	    for(int k = 0; k < n; k++)
	      {
		sum1 = 0.0;
		...
		for(int l = 0; l < n; l++)
		  {
		    if(l < k+1)
		      sum1 = sum1 + fff[k][l][ir]*cos(M_PI*j*(l+0.5)/n);
		    else
		      sum1 = sum1 + fff[l][k][ir]*cos(M_PI*j*(l+0.5)/n);
		  }

		sum2 = sum2 + sum1*cos(M_PI*i*(k+0.5)/n);

	      }

	    //std::cout << "Dans Cheb " << fff[10][11] << std::endl; 
	  
	    // Normalisation de la somme
	    if(i != 0 && j != 0) coef_fff[i][j][ir] = 4*fac*sum2;
	    if(i == 0 && j != 0) coef_fff[i][j][ir] = 2*fac*sum2;
	    if(i != 0 && j == 0) coef_fff[i][j][ir] = 2*fac*sum2;
	    if(i == 0 && j == 0) coef_fff[i][j][ir] = fac*sum2;
	  }
      }
...
double Chebychev::chebev2D(std::vector<std::vector<std::vector<double>>> const& coef_fff,
			   double const& x, double const& y, int ir)
{
  double dd=0.0,ddd=0.0,sv,y1,y2;
  ...
  for(int i = 0; i < n; i++)
    {
      ...
      y2=2.0*(y1=(2.0*y-xmin-xmax)/(xmax-xmin)); //Change of variable.
      
      for (int j=n-1;j>=1;j--) // Clenshaw’s recurrence.
	{
	  sv=dd;
	  if(j<i+1)
	    dd=y2*dd-ddd+coef_fff[i][j][ir];
	  else
	    dd=y2*dd-ddd+coef_fff[j][i][ir];
	  ddd=sv;
	}
      
      coeff_tilde[i] = y1*dd-ddd+coef_fff[i][0][ir];// Last step is different.
    }
  
  return chebev(coeff_tilde, x);
}
