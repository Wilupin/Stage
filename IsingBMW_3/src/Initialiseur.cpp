  ...
  for(int ir = 0; ir <= gamma.get_nrho(); ir++)
	...
	gamma.set_W_rho( 2*(d+mu) - sqrt(2*beta/rho) * (d+mu) * tanh( 2*sqrt(beta)*(d+mu)*sqrt(2*rho)), ir);
      */
      ...
      gamma.set_V_rho( (delta*delta / (1+mu))*rho - log(cosh(2*delta*sqrt(beta)*sqrt(rho)))  , ir);
		       
      if(ir == 0)
	gamma.set_W_rho( delta*delta*(1/(1+mu) - 2*beta), ir); 
      else
	gamma.set_W_rho( delta*delta/(1+mu) - delta*sqrt(beta/rho) * tanh(2*delta*sqrt(beta*rho)), ir);	       
